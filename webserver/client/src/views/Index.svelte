...
<IndexNavbar />
<section
  class="header relative pt-16 items-center flex h-screen max-h-860-px bg-blue-600"
>
  <div class="container mx-auto items-center flex flex-wrap">
    <div class="w-full md:w-8/12 lg:w-6/12 xl:w-6/12 px-4">
      <div class="pt-32 sm:pt-0">
        <h2 class="font-semibold text-4xl text-white">Welcome To Ding Dong!</h2>
        {#if UserRole}
          {#if UserRole === "teacher"}
            <h2 class="font-semibold text-4xl text-white">
              Hello {UserName}! You are a Teacher
            </h2>
          {/if}
          {#if UserRole === "student"}
            <h2 class="font-semibold text-4xl text-white">
              Hello {UserName}! You are a Student
            </h2>
          {/if}
          <h2 class="font-semibold text-3xl text-blueGray-300 mt-4">
            Your User ID: {teacher_id}
          </h2>
        {/if}
        <p class="mt-4 text-lg leading-relaxed text-blueGray-100">
  ...
  <img
    class="absolute top-0 b-auto right-0 pt-16 sm:w-5/12 mt-4 sm:mt-0 w-8/12 max-h-860-px"
    src={patternVue}
    alt="..."
  />
</section>
